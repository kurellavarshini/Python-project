  ...
  for values in image:
      for pixel in values:
          ...
          if data_index < data_len:
              # hide the data into LSB bit of red pixel
              pixel[0] = int(r[:-1] + binary_secret_msg[data_index], 2)
              data_index += 1
          if data_index < data_len:
              # hide the data into LSB bit of green pixel
              pixel[1] = int(g[:-1] + binary_secret_msg[data_index], 2)
              data_index += 1
          if data_index < data_len:
              # hide the data into LSB bit of  blue pixel
              pixel[2] = int(b[:-1] + binary_secret_msg[data_index], 2)
              data_index += 1
          # if data is encoded,  out of the loop
          if data_index >= data_len:
              break
...
def showData(image):
  ...
  for values in image:
      for pixel in values:
          ...
          binary_data += r[-1] #extracting data from the LSB bit of red pixel
          binary_data += g[-1] #extracting data from the LSB bit of red pixel
          binary_data += b[-1] #extracting data from the LSB bit of red pixel
  # split by 8-bits
...
def encode_text(): 
  ...
  cv2.imshow("original image window",resized_image) #display the image
...
def decode_text():
  ...
  cv2.imshow("new image window",resized_image) #display the Steganographed image
